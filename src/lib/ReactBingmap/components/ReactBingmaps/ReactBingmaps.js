...
class ReactBingmaps extends Component {
	...
	componentDidMount() {
		...
		this.setState({
			testing : this.props.showPreviousRoute
		})
	...
	setDirections(directions, mapReference) {
		...
		if (map[mapReference] && directions) {
			...
			Microsoft.Maps.loadModule('Microsoft.Maps.Directions', function () {
				...
				// 	directionsManager._map._layers.clear()
				// }

				// directionsManager.clearAll();
				
				console.log('directionsManager', directionsManager);
				console.log('MicroSoft inputs', Microsoft.Maps.Directions);
				...
				if (requestOptions) {
					...
					let distanceUnit = requestOptions.distanceUnit ?
						...
						null;
						let avoid = requestOptions.avoid ?
						requestOptions.avoid :
						null;
						let optimize = requestOptions.optimize ?
						Microsoft.Maps.Directions.Optimize[requestOptions.optimize] :
						null;
					...
					let routeAvoidance = requestOptions.routeAvoidance ?
						...
						null;
					let routeOptimization = requestOptions.optimize ?
						Microsoft.Maps.Directions.RouteOptimization[requestOptions.optimize] :
						null;
				...
				for (var wayPointsIndex = 0; wayPointsIndex < wayPoints.length; wayPointsIndex++) {
					...
					let location = wayPoints[wayPointsIndex].location
						...
						: null;
					let isViaPoint = (wayPoints[wayPointsIndex].isViaPoint !== undefined && wayPoints[wayPointsIndex].isViaPoint !== null ? wayPoints[wayPointsIndex].isViaPoint : false);
